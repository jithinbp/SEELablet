...
import packet_handler_wifi as packet_handler
...
class Interface(object):
	...
	* loads calibration values.
	...
	def __init__(self,timeout=1.0,**kwargs):
		...
		self.data_splitting = kwargs.get('data_splitting',20)
	...
	def capture_multiple(self,samples,tg,*args):
		...
		if samples%self.data_splitting:
			...
			rem = 2*(samples%self.data_splitting)+1
			for a in range(200):
				partial = self.H.fd.recv(rem)		#reading int by int sometimes causes a communication error. this works better.
				rem -=len(partial)
				data+=partial
	...
	def __calcDChan__(self,name):
		"""
		accepts a string represention of a digital input ( 'ID1','ID2','ID3','ID4','CH1','Fin' ) and returns a corresponding number
		"""
	...
	def capture_edges1(self,waiting_time=1.,**args):
		...
		**Arguments** 
		...
		::
			>>> I.LA_capture1(0.2,channel = 'ID1', trigger_channel='ID1',channel_mode = 3,trigger_mode = 3)  #captures rising edges only. with rising edge trigger on ID1
	...
	def load_waveform(self,num,function,span):
		...
		**Arguments** 
		...
		example::
		  ...
		  self.I.load_waveform(fn,[0,100])  #Load triangular wave to wavegen 1
		  
		  self.I.load_waveform(2,np.sin,[0,2*np.pi]) #Load sinusoidal wave to wavegen 2
